...
boundary-kernel defect and an invalid initial convergence-study design; corrected, the
study recovers the expected second-order accuracy. We report benchmarks from $8^3$ to
$512^3$ cells on stated hardware, with Nsight Compute measurements confirming the
memory-bound conclusion directly, showing a CPU/GPU crossover between $16^3$ and
$32^3$ cells and a speedup plateau around $60$--$68\times$.
\end{abstract}
...
$\mathrm{Fo} = \alpha t/\sigma_0^2$ shrank as $1/n^2$, so the blob diffused less at
...
with OpenMP enabled for the CPU backend.
...
peak bandwidth. Fig.~\ref{fig:roofline} plots this directly: the kernel's fixed
$\mathrm{OI}=0.5$ sits far left of the ridge point, and its achieved performance
(computed from $\mathrm{OI}$ and the \emph{measured} DRAM bandwidth below, not an
independently measured FLOP count) climbs toward, but never reaches, the ceiling the
memory roofline imposes at that intensity.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/roofline.pdf}
\caption{Roofline model for the RTX 3070. The kernel's fixed naive operational
intensity (dashed line) sits far below the ridge point; achieved performance at each
grid size climbs toward the ceiling that intensity imposes as the grid grows.}
\label{fig:roofline}
\end{figure}
...
individually (DRAM throughput, compute throughput, L2 hit rate, achieved
...
$91.1\%$ ($408\,\text{GB/s}$) at $512^3$, while compute throughput stays in a narrow
$43$--$50\%$ band from $64^3$ onward and never approaches saturation, strong evidence
that the kernel is memory-bandwidth-limited rather than compute-limited, not merely
an inference from operational intensity (Table~\ref{tab:ncu}, Fig.~\ref{fig:utilization}).
At $8^3$ and $16^3$, both DRAM and compute utilization are below $4\%$ and occupancy
...
\toprule
Grid size & DRAM \% & GB/s & Compute \% & L2 hit \% & Occupancy \% \\
\midrule
...
\caption{DRAM and compute throughput (\% of peak) vs. grid size. DRAM throughput
overtakes and pulls away from compute throughput beyond $64^3$ cells.}
...
above, which profiler measurements show is consistent with the kernel's low naive
operational intensity ($\approx 0.5$ FLOP/byte): DRAM throughput reaches $91\%$ of
peak at $512^3$ cells while compute throughput never exceeds $50\%$.
...
available at \href{https://github.com/karl-kes/3d-heat-solver}{github.com/karl-kes/3d-heat-solver}.
